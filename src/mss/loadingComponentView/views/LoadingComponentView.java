...
import mss.common.utils.Cuarter;
 ...
 * 
 */
public class LoadingComponentView extends RelativeLayout implements
		ILoadingComponentView {
	...
	private void init(Context context) {
		...
		View container = inflater.inflate(R.layout.layout_loading_component,
				this, true);
		View imageContainer = container
				.findViewById(R.id.backgroundImageContainer);
		backgroudImageView = (ImageView) imageContainer
				.findViewById(R.id.backgroundImage);
		progressBarView = (ProgressBar) container	
				.findViewById(R.id.progress_bar_indicator);
		loadingTextView = (TextView) container
				.findViewById(R.id.loading_textview);
		blockingMode = false;
		millisBetweenFrames = 500;
		defaultImageResource = R.drawable.mss_logo;
	...
	public void addPhase(String phase, Queue<Integer> listImages,
		...
		if (listImages != null) {
			getPhases().add(
					new Cuarter<String, Queue<Integer>, String, Integer>(phase,
							listImages, loadingText, progress));
			...
			getPhases().add(
					new Cuarter<String, Queue<Integer>, String, Integer>(phase,
							defaultList, loadingText, progress));
		}
	...
	private Cuarter<String, Queue<Integer>, String, Integer> getCurrentPhase() {
		Cuarter<String, Queue<Integer>, String, Integer> phaseName = new Cuarter<String, Queue<Integer>, String, Integer>(
				null, null, null, null);
	...
	private void nextPhase() {
		if (getPhases().size() > 0) {
			Cuarter<String, Queue<Integer>, String, Integer> currentPhase = getPhases()
					.get(0);
	...
	private void nextFrame(final String phaseName) {
		...
		if (image == null) {
			...
			backgroudImageView.setBackgroundResource(image);
